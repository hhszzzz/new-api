...
type ChannelOtherSettings struct {
	...
	AllowAlphaSearch                      bool                  `json:"allow_alpha_search,omitempty"`         // 是否允许默认 OpenAI 渠道转发 /v1/alpha/search（默认关闭）
	...
	DifyRequireSuccessfulWorkflow         bool                  `json:"dify_require_successful_workflow,omitempty"`
...
func (r AdvancedCustomRoute) ResolveTarget() (relayconvert.Protocol, error) {
	if strings.TrimSpace(r.Converter) == AdvancedCustomConverterSGLangRerank {
		source, _ := relayconvert.DetectOperation(r.IncomingPath)
		return source.Protocol, nil
	}
	return relayconvert.ResolveTarget(r.IncomingPath, r.TargetProtocol, r.Converter)
}

// SupportsPassThroughBody reports whether a route keeps the incoming protocol.
func (r AdvancedCustomRoute) SupportsPassThroughBody() bool {
	if r.Converter == AdvancedCustomConverterSGLangRerank {
		return true
	}
	target, err := r.ResolveTarget()
	source, _ := relayconvert.DetectOperation(r.IncomingPath)
	return err == nil && target == source.Protocol
}
...
func (r AdvancedCustomRoute) IsNative() bool {
	return r.Converter != AdvancedCustomConverterSGLangRerank && r.SupportsPassThroughBody()
}
...
func IsAdvancedCustomConverterAllowed(converter string) bool {
	if converter == "" || converter == relayconvert.ConverterNone || converter == AdvancedCustomConverterSGLangRerank {
		return true
	}
