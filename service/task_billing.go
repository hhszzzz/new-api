...
func LogTaskConsumption(c *gin.Context, info *relaycommon.RelayInfo, task *model.Task) {
	...
	AppendModelRoutingAdminInfo(other, info.HasModelRouting(), info.UpstreamModelName)
	if snap := info.TieredBillingSnapshot; snap != nil {
...
func taskBillingOther(task *model.Task) *model.LogOther {
	...
	if props.UpstreamModelName != "" && props.UpstreamModelName != props.OriginModelName {
		AppendModelRoutingAdminInfo(other, true, props.UpstreamModelName)
	}
	appendTaskLogInfo(task, other)
	return other
}
...
func RecalculateTaskQuotaByTokens(ctx context.Context, task *model.Task, totalTokens int) bool {
	...
	if billingContext := task.PrivateData.BillingContext; billingContext != nil && billingContext.ModelRatio > 0 && billingContext.GroupRatio > 0 {
		...
		modelRatioSetting, hasRatioSetting, _ := ratio_setting.GetModelRatio(taskModelName(task))
		if !hasRatioSetting {
			return false
		}
		modelRatio = modelRatioSetting

		executionGroup := task.Group
		userGroup := ""
		if user, err := model.GetUserById(task.UserId, false); err == nil {
			userGroup = user.Group
			if executionGroup == "" {
				executionGroup = userGroup
			}
		}
		if executionGroup == "" {
			return false
		...
		finalGroupRatio = ratio_setting.GetGroupRatio(executionGroup)
		if specialRatio, ok := ratio_setting.GetGroupGroupRatio(userGroup, executionGroup); ok {
