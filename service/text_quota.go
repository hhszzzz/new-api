...
func collectToolSurchargeItem(items []ToolSurchargeItem, toolPrices *operation_setting.ToolPriceSnapshot, name string, count int, modelName string) []ToolSurchargeItem {
	...
	price := toolPrices.GetToolPriceForModel(name, modelName)
...
func calculateTextToolCallSurcharge(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, summary *textQuotaSummary) decimal.Decimal {
	modelName := summary.BillingModelName
	if modelName == "" {
	...
	if relayInfo.ResponsesUsageInfo != nil {
		for name, tool := range relayInfo.ResponsesUsageInfo.BuiltInTools {
			...
			items = collectToolSurchargeItem(items, toolPrices, name, tool.CallCount, modelName)
		}
	}
	if relayInfo.RelayMode != relayconstant.RelayModeResponses &&
		strings.HasSuffix(summary.ModelName, "search-preview") {
		items = collectToolSurchargeItem(items, toolPrices, dto.BuildInToolWebSearchPreview, 1, modelName)
	}
	...
	if ctx.GetBool("gemini_google_search_call") {
		items = collectToolSurchargeItem(items, toolPrices, dto.BuildInToolGoogleSearch, 1, modelName)
	}
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	relayInfo.PerformanceCacheHitTokens = summary.CacheHitTokens
	relayInfo.PerformanceCacheMissTokens = summary.CacheMissTokens
}
