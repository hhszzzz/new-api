...
func TestGetWaffoPancakePayMoney(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		setTopUpPricingSettingsForTest(t, originalGlobalPrice, originalQuotaDisplayType, originalDiscounts)
		require.NoError(t, common.UpdateTopupGroupRatioByJSONString(originalTopupGroupRatio))
	})

	setting.WaffoPancakeUnitPrice = 99
	setTopUpPricingSettingsForTest(t, 2.5, operation_setting.QuotaDisplayTypeUSD, map[int]float64{
		10:                                0.8,
		int(common.GetQuotaPerUnit() * 3): 0.5,
		20:                                0,
	})
	...
	testCases := []struct {
		...
		},
		{
			...
			amount:           int64(common.GetQuotaPerUnit() * 3),
			group:            "vip",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			setTopUpPricingSettingsForTest(t, 2.5, tc.quotaDisplayType, map[int]float64{
				10:                                0.8,
				int(common.GetQuotaPerUnit() * 3): 0.5,
				20:                                0,
			})
