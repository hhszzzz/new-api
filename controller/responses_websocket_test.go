...
func setupResponsesWSRequestTest(t *testing.T) (*model.User, *model.Token) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Token{}, &model.UserGroupMembership{}, &model.UserModelPermission{}, &model.UserModelBlock{}, &model.UserModelRoute{}, &model.UserModelRouteGroup{}, &model.UserModelRouteChannel{}, &model.UserModelRouteExecutionGroup{}, &model.UserSubscription{}))
	...
	user.AffCode = fmt.Sprintf("responses-ws-%d", user.Id)
...
func TestResponsesWSRequestRunnerRefreshesBillingContextAndCleansBody(t *testing.T) {
	...
	require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(`{"default":1,"updated":1}`))
	...
	require.Nil(t, runner(request, "ws-first", func(c *gin.Context) *hosttypes.NewAPIError {
	...
	require.NoError(t, model.DB.Create(&model.UserGroupMembership{UserId: user.Id, GroupName: "updated"}).Error)
	request = httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"input":"second"}`))
	require.Nil(t, runner(request, "ws-second", func(c *gin.Context) *hosttypes.NewAPIError {
...
func TestResponsesWSRequestRunnerRejectsRevokedCredentials(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "before", func(*gin.Context) *hosttypes.NewAPIError { return nil }))
			tc.update(t, user, token)
			called := false
			apiError := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "after", func(*gin.Context) *hosttypes.NewAPIError {
				called = true
				return nil
			})
...
func TestResponsesWSRequestRunnerUsesExistingMemoryRateLimit(t *testing.T) {
	...
	runner, _ := newResponsesWSTestRunner(t, token)
	called := 0
	handle := func(*gin.Context) *hosttypes.NewAPIError { called++; return nil }
	...
	assert.Contains(t, apiError.Error(), "request id: limited")
...
func TestResponsesWSRequestRunnerSharesRedisSuccessLimitWithHTTP(t *testing.T) {
	...
	runner, _ := newResponsesWSTestRunner(t, token)
	failed := hosttypes.NewErrorWithStatusCode(errors.New("invalid input"), hosttypes.ErrorCodeInvalidRequest, http.StatusBadRequest)
	assert.Same(t, failed, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "failed", func(*gin.Context) *hosttypes.NewAPIError { return failed }))
	require.Nil(t, runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "successful", func(*gin.Context) *hosttypes.NewAPIError { return nil }))
	engine := gin.New()
	called := false
...
type responsesWSBillingTest struct {
	...
	gatewayURL                  string
	perfRequests, perfSuccesses int64
}
...
// Health samples are stored synchronously in the local instance bucket. Compare
// against the fixture baseline because the metrics collector outlives its database.
func waitPerfCounters(t *testing.T, fixture *responsesWSBillingTest, wantRequests int64) (requests, successes int64) {
	t.Helper()
	summary, err := perfmetrics.QuerySummaryAll(1, []string{"default"})
	require.NoError(t, err)
	for _, item := range summary.Models {
		if item.ModelName == "ws-billing" {
			requests = item.RequestCount
			successes = int64(math.Round(float64(requests) * item.SuccessRate / 100))
		}
	}
	requests -= fixture.perfRequests
	successes -= fixture.perfSuccesses
...
func newResponsesWSBillingTest(t *testing.T, expression string, handle func(*websocket.Conn, *http.Request), httpEvents ...string) *responsesWSBillingTest {
	...
	previousBatch, previousLogs, previousCount, previousQuota := common.BatchUpdateEnabled, common.LogConsumeEnabled, constant.CountToken, common.GetQuotaPerUnit()
	common.BatchUpdateEnabled, common.LogConsumeEnabled, constant.CountToken = false, true, false
	require.NoError(t, common.SetQuotaPerUnit(500000))
	...
	t.Cleanup(func() {
		common.BatchUpdateEnabled, common.LogConsumeEnabled, constant.CountToken = previousBatch, previousLogs, previousCount
		require.NoError(t, common.SetQuotaPerUnit(previousQuota))
	...
	require.NoError(t, model.DB.AutoMigrate(&model.Channel{}, &model.Ability{}, &model.PerfMetric{}, &model.PerfMetricInstance{}))
	...
	for _, item := range summary.Models {
		if item.ModelName == "ws-billing" {
			fixture.perfRequests = item.RequestCount
			fixture.perfSuccesses = int64(math.Round(float64(item.RequestCount) * item.SuccessRate / 100))
		}
	}
	redisServer := miniredis.RunT(t)
	...
	channel.SetOtherSettings(hostdto.ChannelOtherSettings{AllowServiceTier: true})
...
func TestResponsesInterruptedStreamHealth(t *testing.T) {
	for _, scenario := range []string{"websocket timeout", "sse timeout", "sse client cancel"} {
		t.Run(scenario, func(t *testing.T) {
			...
			if scenario == "sse client cancel" {
				requests, successes := waitPerfCounters(t, fixture, 0)
				assert.Zero(t, requests, "client cancellation must not affect model health")
				assert.Zero(t, successes)
				return
			}
			requests, successes := waitPerfCounters(t, fixture, 1)
...
func responsesSSETerminal(t *testing.T, body string) []byte {
	t.Helper()
	var terminal []byte
	...
	require.NotEmpty(t, terminal, "stream must contain an explicit terminal event")
	return terminal
}
...
func TestResponsesWebSocketDialsNativeResponsesChannelTypes(t *testing.T) {
	...
	queryAuthRoute := hostdto.ChannelOtherSettings{AdvancedCustom: &hostdto.AdvancedCustomConfig{Routes: []hostdto.AdvancedCustomRoute{{
		IncomingPath: "/v1/responses", UpstreamPath: "/upstream/responses",
		Auth: &hostdto.AdvancedCustomRouteAuth{Type: hostdto.AdvancedCustomAuthTypeQuery, Name: "api_key", Value: "{api_key}"},
	}}}}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.JSONEq(t, terminal, string(responsesSSETerminal(t, string(body))))
			select {
			case <-fixture.httpDone:
			case <-time.After(3 * time.Second):
...
func TestResponsesSSERejectionBeforeAcceptanceRefundsFixedPrice(t *testing.T) {
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(_ *websocket.Conn, _ *http.Request) {
		t.Error("a rejected HTTP request must not open an upstream WebSocket")
	}, `{"type":"error","code":"unsupported_endpoint","message":"unsupported endpoint /v1/responses"}`)
	request, err := http.NewRequest(http.MethodPost, fixture.gatewayURL+"/v1/responses", strings.NewReader(`{"model":"ws-billing","input":"hi","stream":true}`))
	require.NoError(t, err)
	request.Header.Set("Authorization", "Bearer sk-"+fixture.token.Key)
	request.Header.Set("Content-Type", "application/json")
	...
	require.NoError(t, err)
	require.NoError(t, response.Body.Close())
	assert.Contains(t, string(body), "unsupported_endpoint")
	fixture.closeAndWait(t)
	// BillingSession refunds the wallet and token asynchronously. Wait for
	// both durable balances before closing the fixture's database connection.
	require.EventuallyWithT(t, func(collect *assert.CollectT) {
		var token model.Token
		var user model.User
		if !assert.NoError(collect, model.DB.First(&token, fixture.token.Id).Error) ||
			!assert.NoError(collect, model.DB.First(&user, fixture.user.Id).Error) {
			return
		}
		assert.Equal(collect, 3000, token.RemainQuota)
		assert.Equal(collect, 100000, user.Quota)
			...
			t.Run(transport+"/"+tc.name, func(t *testing.T) {
				...
				if transport == "http-sse" {
					...
					if tc.failed {
						...
						require.NoError(t, common.Unmarshal(responsesSSETerminal(t, string(body)), &failure))
						assert.Equal(t, "error", failure["type"])
						assert.NotEmpty(t, failure["code"])
						assert.NotContains(t, string(body), "event: response.completed")
					} else {
						assert.JSONEq(t, terminal, string(responsesSSETerminal(t, string(body))))
					}
				} else {
				...
				if tc.failed {
					...
					second := readResponsesWSTestEvent(t, fixture.client)
					assert.Equal(t, "response.completed", second["type"], "response: %v", second)
				...
				requests, successes := waitPerfCounters(t, fixture, expectedRequests)
