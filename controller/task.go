...
func GetAllTask(c *gin.Context) {
	...
	audience := relay.TaskDtoAudiencePublic
	if canViewModelRouting(c) {
		audience = relay.TaskDtoAudienceAdmin
	}
	pageInfo.SetItems(tasksToDto(items, true, audience, c.GetInt("role")))
...
func GetUserTask(c *gin.Context) {
	...
	pageInfo.SetItems(tasksToDto(items, false, audience, c.GetInt("role")))
	common.ApiSuccess(c, pageInfo)
}

func tasksToDto(tasks []*model.Task, fillUser bool, audience relay.TaskDtoAudience, viewerRole int) []*dto.TaskDto {
	...
	for i, task := range tasks {
		...
		item := relay.TaskModel2Dto(task, audience)
