...
func TestTaskLogDTOSeparatesUserAdminAndRootDetails(t *testing.T) {
	...
	userView := tasksToDto([]*model.Task{task}, false, relay.TaskDtoAudiencePublic, common.RoleCommonUser)[0]
	assert.Nil(t, userView.AdminInfo)
	assert.Nil(t, userView.RootInfo)

	adminView := tasksToDto([]*model.Task{task}, false, relay.TaskDtoAudienceAdmin, common.RoleAdminUser)[0]
	...
	rootView := tasksToDto([]*model.Task{task}, false, relay.TaskDtoAudienceAdmin, common.RoleRootUser)[0]
...
func TestTaskLogDTOReplacesLegacyVideoURLWithAvailabilityFlag(t *testing.T) {
	...
	view := tasksToDto([]*model.Task{task}, false, relay.TaskDtoAudiencePublic, common.RoleCommonUser)[0]
...
func TestTaskLogDTOKeepsFailureReasonAndDoesNotMarkPluginTaskLegacy(t *testing.T) {
	...
	failedView := tasksToDto([]*model.Task{failed}, false, relay.TaskDtoAudiencePublic, common.RoleCommonUser)[0]
	...
	pluginView := tasksToDto([]*model.Task{pluginTask}, false, relay.TaskDtoAudiencePublic, common.RoleCommonUser)[0]
