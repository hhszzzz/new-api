...
func TestPreConsumePolicyDatabaseMatrix(t *testing.T) {
	...
	previousUnit, previousBatch := common.GetQuotaPerUnit(), common.BatchUpdateEnabled
	require.NoError(t, common.SetQuotaPerUnit(500000))
	common.BatchUpdateEnabled = false
	t.Cleanup(func() {
		require.NoError(t, config.GlobalConfig.LoadFromDB(previousConfig))
		require.NoError(t, common.SetQuotaPerUnit(previousUnit))
	...
	for _, dialect := range []struct{ kind, env string }{{"sqlite", ""}, {"mysql", "TEST_MYSQL_DSN"}, {"postgres", "TEST_POSTGRES_DSN"}} {
		t.Run(dialect.kind, func(t *testing.T) {
			...
			for range 2 {
				...
				quotaSetting.TrustQuotaUSD = 10
				quotaSetting.PreConsumeMultiplier = 1
...
func TestPreConsumeMultiplierRejectsInvalidRuntimeAndOverflow(t *testing.T) {
	...
	for _, multiplier := range []float64{0, -0.5, math.NaN(), math.Inf(1), math.MaxFloat64} {
		quotaSetting := operation_setting.GetQuotaSetting()
		quotaSetting.PreConsumeMultiplier = multiplier
		quotaSetting.PublishConfig()
		ctx, _ := gin.CreateTestContext(httptest.NewRecorder())
		ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", nil)
