...
func TestSameChannelPinsMergeToStricterRetryMode(t *testing.T) {
	...
	assert.False(t, service.ShouldRetryRelayError(c, hosttypes.NewOpenAIError(errors.New("upstream"), hosttypes.ErrorCodeBadResponseStatusCode, http.StatusInternalServerError), 1))
}
					...
					for _, tc := range []struct {
						...
						t.Run(fmt.Sprintf("cache=%t/keep=%t/global=%s/rule=%s", cached, keep, tc.globalMode, tc.ruleMode), func(t *testing.T) {
							...
							*affinity = snapshot.Affinity
							...
							request.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"policy-test","messages":[{"role":"user","content":"hi"}]}`))
