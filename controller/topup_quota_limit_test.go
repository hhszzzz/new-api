...
func setTopUpTestQuotaDisplayType(displayType string) {
...
func TestTopUpQuotaValidation(t *testing.T) {
	...
	t.Cleanup(func() {
		require.NoError(t, common.SetQuotaPerUnit(oldQuotaPerUnit))
		setTopUpTestQuotaDisplayType(oldDisplayType)
	})
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			setTopUpTestQuotaDisplayType(tc.displayType)
...
func TestValidateTopUpQuotaReturnsMaximumAmount(t *testing.T) {
	...
	oldDisplayType := operation_setting.GetGeneralSetting().QuotaDisplayType
	require.NoError(t, common.SetQuotaPerUnit(500000))
	setTopUpTestQuotaDisplayType(operation_setting.QuotaDisplayTypeUSD)
	t.Cleanup(func() {
		require.NoError(t, common.SetQuotaPerUnit(oldQuotaPerUnit))
		setTopUpTestQuotaDisplayType(oldDisplayType)
	})
