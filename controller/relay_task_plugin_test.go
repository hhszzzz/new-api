...
func TestImmediateTaskSettlementDatabase(t *testing.T) {
	db, dialect := openTaskDialectDatabase(t, &model.User{}, &model.UserSubscription{}, &model.Channel{}, &model.Task{}, &model.Log{})
...
`
	...
	for index, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer server.Close()
			initial := int(20 * common.GetQuotaPerUnit())
			...
			require.Nil(t, taskErr)
			require.NotNil(t, outcome)
			want := common.QuotaRound(tc.count * 0.01 * common.GetQuotaPerUnit())
			...
			assert.Equal(t, float64(4), info.TieredBillingSnapshot.EstimatedQuotaBeforeGroup/(0.01*common.GetQuotaPerUnit()))
