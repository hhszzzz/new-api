...
func GetBillingExpr(model string) (string, bool) {
	if expr, ok := billingSettingSnapshot.Load().BillingExpr[model]; ok {
		return expr, true
	}
...
func GetBillingModeCopy() map[string]string {
	modes := lo.Assign(billingSettingSnapshot.Load().BillingMode)
...
func GetBillingExprCopy() map[string]string {
	expressions := lo.Assign(billingSettingSnapshot.Load().BillingExpr)
	for model := range builtinBillingExpr {
