...
func ResponsesResponseToClaudeMessagesResponse(resp *dto.OpenAIResponsesResponse) (*dto.ClaudeResponse, *dto.Usage, error) {
	...
	for index := range resp.Output {
		...
		case responsesOutputTypeReasoning:
			if thinking := extractReasoningTextFromOutput(&output); thinking != "" {
		...
		case responsesOutputTypeMessage:
			for _, content := range output.Content {
				if content.Type == "refusal" {
					claudeResponse.Content = append(claudeResponse.Content, dto.ClaudeMediaMessage{Type: "text", Text: kitutil.GetPointer(content.Refusal)})
					continue
				}
		...
		case responsesOutputTypeFunctionCall, responsesOutputTypeCustomToolCall, responsesOutputTypeToolSearchCall:
			...
			claudeResponse.Content = append(claudeResponse.Content, dto.ClaudeMediaMessage{
				...
				Id:    callID,
				Name:  name,
				Input: responsesArgumentsToClaudeInput(responsesToolCallArguments(&output)),
			})
		}
	}
