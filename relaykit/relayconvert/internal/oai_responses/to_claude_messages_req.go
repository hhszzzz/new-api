...
func OpenAIResponsesRequestToClaudeMessages(c context.Context, info convmeta.Meta, req *dto.OpenAIResponsesRequest) (*dto.ClaudeRequest, error) {
	...
	for _, item := range inputItems {
		...
		case "additional_tools":
			continue
		case "reasoning":
			// Provider-specific reasoning state cannot be forwarded to Claude
			// without a valid Anthropic signature. Preserve reasoning settings on
			// the request, but do not turn visible or encrypted reasoning items
