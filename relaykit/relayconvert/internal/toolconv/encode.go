...
func attachOpenAIResponsesRequest(request any, set Set) (any, []types.ConversionDiagnostic, error) {
	...
	for index, definition := range set.Definitions {
		...
		case KindFunction:
			...
			// A protocol converter can synthesize a declaration for historical
			// calls while the original declarations are in this tool set. Prefer
			// the original schema when restoring the matching function.
			for i := 0; i < len(tools); {
				existing, ok := tools[i].(map[string]any)
				if ok && existing["type"] == "function" && existing["name"] == definition.Function.Name {
