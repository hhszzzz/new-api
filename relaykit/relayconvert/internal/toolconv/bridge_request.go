...
func ExtractRequestForConversion(format types.RelayFormat, request any) (any, Set, error) {
	if format != types.RelayFormatOpenAIResponses { return ExtractRequest(format, request) }
	source, ok := request.(*dto.OpenAIResponsesRequest)
	if !ok {
		value, valid := request.(dto.OpenAIResponsesRequest)
		if !valid { return nil, Set{}, fmt.Errorf("expected Responses request, got %T", request) }
		source = &value
	}
	var tools []json.RawMessage
	if len(source.Tools) > 0 && kitutil.GetJsonType(source.Tools) != "null" {
		if err := kitutil.Unmarshal(source.Tools, &tools); err != nil { return nil, Set{}, fmt.Errorf("invalid Responses tools: %w", err) }
	}
	var clientTools, hostedTools []json.RawMessage
	for _, raw := range tools {
		if kitutil.GetJsonType(raw) == "string" { clientTools = append(clientTools, raw); continue }
		var tool struct { Type string `json:"type"` }
		if err := kitutil.Unmarshal(raw, &tool); err != nil { return nil, Set{}, err }
	...
	if len(hostedTools) > 0 { hosted.Tools, _ = kitutil.Marshal(hostedTools) }
	clientChoice := source.ToolChoice
	if len(source.ToolChoice) > 0 && kitutil.GetJsonType(source.ToolChoice) == "object" {
		var choice struct { Type string `json:"type"` }
		if err := kitutil.Unmarshal(source.ToolChoice, &choice); err != nil { return nil, Set{}, err }
		switch choice.Type {
	...
	if len(clientTools) > 0 { prepared.Tools, _ = kitutil.Marshal(clientTools) }
