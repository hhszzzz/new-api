  ...
  if (isTieredExpr && other.is_task) {
    ...
    if (tieredSummary) {
      ...
      const otherEntries = tieredSummary.priceEntries
        ...
        .map((entry) =>
          entry.unit
            ? `${tierLabel(tieredSummary.tier.label)} · ${t(entry.shortLabel)} ${formatPriceCompact(entry.price)}/${t(entry.unit)}`
...
export function useCommonLogsColumns(
    ...
    columns.push(
      {
        ...
        header: t('Client'),
        accessorFn: (row) =>
          parseLogOther(row.other)?.diagnostics?.client ?? '',
        cell: ({ row }) => {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const ip =
            row.original.ip ||
            parseLogOther(row.original.other)?.diagnostics?.ip
          return ip ? (
  ...
  }, [t, isAdminView, isRoot, canViewModelRoute, showBillingSource, currency])
}
