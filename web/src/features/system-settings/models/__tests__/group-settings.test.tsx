...
describe('group settings workspace', () => {
  ...
  it('preserves ratio rule state across keyboard tabs without restoring legacy authorization controls', async () => {
    const user = userEvent.setup()
    ...
    render(
      <Fixture
        onSave={onSave}
        initial={{
          GroupGroupRatio: '{"vip":{"default":0.5}}',
    ...
    expect(
      screen.queryByRole('tab', { name: 'Group visibility' })
    ).not.toBeInTheDocument()
    ...
    expect(onSave.mock.calls[0][0]).toEqual(
      expect.objectContaining({
        GroupGroupRatio: '{"vip":{"default":0.5}}',
        GroupSpecialUsableGroup: '{"vip":{"+:default":"Standard"}}',
        TopupGroupRatio: defaults.TopupGroupRatio,
      })
    )
  })
  ...
  it('preserves pricing edits when switching between visual and JSON editors', async () => {
    const user = userEvent.setup()
    render(<Fixture />)
    await user.clear(screen.getByRole('textbox', { name: 'Description: vip' }))
    await user.type(
      screen.getByRole('textbox', { name: 'Description: vip' }),
      'Updated description'
    )
