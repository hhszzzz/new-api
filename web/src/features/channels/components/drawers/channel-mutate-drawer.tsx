...
export function ChannelMutateDrawer({
  ...
  const currentAggregateId = formValues.aggregate_id
  const currentAggregate = getChannelAggregateById(
    aggregatesData?.data ?? [],
    currentAggregateId
  )
  const currentInheritAggregateBaseURL = formValues.inherit_aggregate_base_url
  const currentClientPolicyMode = formValues.client_policy_mode
  const currentClientPolicyClients = formValues.client_policy_clients
  ...
  useEffect(() => {
    ...
    if (isEditing && channelData?.data?.id === channelId) {
        ...
        <FormItem className='flex items-center justify-between px-4 py-3'>
          ...
          <FormControl>
            <Switch
              disabled={
                sensitiveLocked ||
                parseProtocolPolicy(field.value ?? '') === null
              }
              checked={
                field.value === undefined
                  ? formValues.pass_through_body_enabled === true
                  : parseProtocolPolicy(field.value)?.request_mode ===
                    'passthrough'
              }
              onCheckedChange={(value) => {
                const apply = (enabled: boolean) => {
                  if (form.getValues('protocol_policy_json') === undefined) {
                  ...
                  const policy = parseProtocolPolicy(
                    form.getValues('protocol_policy_json') ?? ''
                  )
                  if (policy) {
                    field.onChange(
                      JSON.stringify({
                        ...policy,
                        request_mode: enabled ? 'passthrough' : 'structured',
                      })
                    )
                  }
                }
      ...
      <ChannelConfiguration
        ...
        connection={
          <>
            ...
            <fieldset
              disabled={sensitiveLocked}
              className='space-y-4 rounded-lg border p-4 disabled:opacity-60'
            >
              <FormField
                control={form.control}
                name='aggregate_id'
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Select
                        ...
                        onValueChange={(value) =>
                          (() => {
                            ...
                            if (!nextId) {
                              form.setValue('inherit_aggregate_base_url', false)
                            }
                          })()
                        }
                        disabled={sensitiveLocked}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t('No aggregate')}>
                            {currentAggregate?.name ?? t('No aggregate')}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          ...
                          {(aggregatesData?.data ?? []).map((aggregate) => (
                            <SelectItem
                              key={aggregate.id}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className={sideDrawerSwitchItemClassName()}>
                    <div>
                      <FormLabel>{t('Inherit aggregate base URL')}</FormLabel>
                      <FormDescription className='text-xs'>
                        {t(
                    ...
                    <FormControl>
                      <Switch
                        checked={field.value === true}
                        onCheckedChange={field.onChange}
                        disabled={sensitiveLocked || !currentAggregateId}
                      />
                    </FormControl>
                  </FormItem>
                )}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <ClientMultiSelect
                        ...
                        disabled={currentClientPolicyMode === 'unrestricted'}
                      />
                    </FormControl>
                    <FormDescription>
                      {t(
            ...
            <div className='space-y-4 rounded-lg border p-4'>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='flex items-center justify-between'>
                    <div className='space-y-0.5'>
                      <FormLabel>{t('Disable Model On Error')}</FormLabel>
                      <FormDescription>
                        {t(
              ...
              {isEditing && channelId && (
                <Button
                  type='button'
                  variant='outline'
                  onClick={() => setDisabledModelsDialogOpen(true)}
                >
                  <Ban />
                ...
                render={({ field }) => (
                  <FormItem>
                    <ProtocolPolicyEditor
                      inherit
                      disabled={sensitiveLocked}
                      value={field.value ?? inheritedProtocolPolicy}
                      onChange={field.onChange}
                    />
                    <FormMessage />
                  </FormItem>
              ...
              {currentType === 1 && (
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
                      <div className='space-y-0.5'>
                        <FormLabel>{t('Allow Alpha Search')}</FormLabel>
                        <FormDescription>
                          {t(
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
                      <div className='space-y-0.5'>
                        <FormLabel>
                          {t('Require successful Dify workflow')}
                        </FormLabel>
                        <FormDescription>
                          {t(
