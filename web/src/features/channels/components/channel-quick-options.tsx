...
import type { ChannelFormInput, ChannelFormValues } from '../lib/channel-form'
import {
...
type ChannelQuickOptionsProps = {
  ...
  form?: UseFormReturn<ChannelFormInput, unknown, ChannelFormValues>
  /**
...
export function ChannelQuickOptions(props: ChannelQuickOptionsProps) {
  ...
  const policy =
    policyJSON === undefined ? undefined : parseProtocolPolicy(policyJSON)
  const applyBodyPassthrough = (value: boolean) => {
    if (policyJSON === undefined) {
      ...
      const current = parseProtocolPolicy(
      ...
      if (current) {
        form.setValue(
          ...
          JSON.stringify({
            ...current,
            request_mode: value ? 'passthrough' : 'structured',
          }),
  ...
  if (props.channelType !== CHANNEL_TYPE_TASK_PLUGIN) {
    options.push({
      ...
      checked:
        policy === undefined
          ? passthrough === true
          : policy?.request_mode === 'passthrough',
      onCheckedChange: (value) => {
        if (!value) {
          applyBodyPassthrough(false)
          return
        }
        void props.confirmEnablePassthrough('body').then((confirmed) => {
          if (confirmed) applyBodyPassthrough(true)
        })
      },
      disabled: sensitiveDisabled || policy === null,
    })
    options.push({
