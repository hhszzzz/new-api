...
export function useRedirectPanelPlacement(
  ...
  useLayoutEffect(() => {
    if (!enabled || window.innerWidth < REDIRECT_PANEL_MIN_VIEWPORT) {
      // eslint-disable-next-line react/set-state-in-effect -- Placement is measured from the committed sheet DOM before paint.
      setPlacement(null)
      return
    }
    const anchor =
      ...
      anchorRef.current
    if (!anchor) {
      // eslint-disable-next-line react/set-state-in-effect -- The anchor may disappear when the sheet closes.
      setPlacement(null)
      return
    }
    ...
    // eslint-disable-next-line react/set-state-in-effect -- Synchronize React state with the measured DOM rectangle.
