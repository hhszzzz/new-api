...
export function useChannelModelDiscovery(props: ChannelModelDiscoveryProps) {
  ...
  if (
    previousRequest.enabled !== props.enabled ||
    previousRequest.request !== props.request
  ) {
    setPreviousRequest({ enabled: props.enabled, request: props.request })
    setState((previous) => {
      if (!props.enabled || previous.status === 'idle') {
