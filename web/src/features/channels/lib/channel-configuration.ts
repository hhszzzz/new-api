...
import { channelFormSchema, type ChannelFormInput } from './channel-form'
...
const CONFIGURATION_BLOCKS = {
  ...
  string,
  {
    ...
    fields: readonly FieldPath<ChannelFormInput>[]
  }
>
...
export function getChannelConfigurationState(
  ...
  const configured: Record<ChannelConfigurationBlock, boolean> = {
    ...
    routingStrategy: Boolean(
      ...
      values.schedule?.weekly_enabled ||
      values.schedule?.starts_at ||
      values.schedule?.expires_at ||
      values.schedule?.paused_until ||
      (values.client_policy_mode &&
        values.client_policy_mode !== 'unrestricted') ||
      values.rpm_limit ||
      values.concurrency_limit ||
      values.disable_model_on_error ||
      (values.auto_ban ?? 1) !== 1
    ),
    overrideRules:
    ...
    requestProcessing: Boolean(
      ...
      (values.type !== CHANNEL_TYPE_ADVANCED_CUSTOM &&
        (values.protocol_policy_json === undefined
          ? values.pass_through_body_enabled
          : parseProtocolPolicy(values.protocol_policy_json)?.request_mode ===
            'passthrough')) ||
      hasProtocolOverrides(values.protocol_policy_json) ||
      values.allow_alpha_search ||
