...
function renderModelDetails() {
  client.setQueryData(['pricing-group-performance', null, null, 'test-model'], {
    success: true,
    data: {
...
describe('server performance summaries', () => {
  ...
  it('feeds the latency chart from the status snapshot rather than group averages', async () => {
    renderModelDetails()
    await waitFor(() => expect(chart).toHaveBeenCalled())
    const specs = chart.mock.calls.map(([props]) => props.spec)
    const values = specs.flatMap((value) =>
      value.data.flatMap((data) => data.values)
    )
    expect(values).toEqual(
      expect.arrayContaining([expect.objectContaining({ ttft: 100 })])
    )
    expect(values).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ ttft: 50 })])
    )
  })
