...
/**
 * One-time migration for the legacy default that hid every date column. The
 * table's persisted-visibility restore filters unknown legacy keys
 * (created_time/accessed_time) out of state before any effect can see them,
 * so the migration must rewrite storage before useDataTable reads it.
 */
function migrateLegacyHiddenDateColumns(storageKey: string) {
  ...
  try {
    ...
    const visibility = parsed as Record<string, unknown>
    if (
      visibility.activity_time === undefined &&
      visibility.created_time === false &&
      visibility.accessed_time === false &&
      visibility.expired_time === false
    ) {
...
export function ApiKeysTable() {
  ...
  // Runs before useDataTable restores persisted visibility (hook order).
  useState(() => {
    migrateLegacyHiddenDateColumns(API_KEYS_COLUMN_VISIBILITY_STORAGE_KEY)
    return null
  })
