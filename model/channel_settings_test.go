		...
		t.Run(dialect, func(t *testing.T) {
			...
			for _, channelType := range []int{constant.ChannelTypeVLLM, constant.ChannelTypeSGLang} {
				t.Run(fmt.Sprint(channelType), func(t *testing.T) {
					...
					settings := hostdto.ChannelOtherSettings{AdvancedCustom: &hostdto.AdvancedCustomConfig{Routes: []hostdto.AdvancedCustomRoute{{IncomingPath: "/v1/chat/completions", UpstreamPath: "/custom/chat", Models: []string{"allowed"}, Auth: &hostdto.AdvancedCustomRouteAuth{Type: "none"}}}}}
					...
					for range 2 {
						...
						for _, tc := range []struct {
							...
							ok, _ := ChannelSatisfiesFilters(&loaded, tc.model, []hostdto.ChannelFilter{{Kind: hostdto.FilterRequestPath, RequestPath: tc.path}})
							assert.Equal(t, tc.allowed, ok)
						}
						endpoints := getPricingEndpointTypesForAbility(AbilityWithChannel{ChannelType: channelType, Ability: Ability{Model: "allowed", ChannelId: loaded.Id}}, map[int]*hostdto.AdvancedCustomConfig{loaded.Id: actual})
