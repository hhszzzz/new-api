...
func TestAppendLogDiagnosticsKeepsOnlySafeRequestMetadata(t *testing.T) {
	...
	common.SetContextKey(context, constant.ContextKeyProtocolStateMode, "none")
	...
	assert.NotContains(t, diagnostics, "upstream_protocol")
	assert.NotContains(t, diagnostics, "protocol_converter")
	...
	assert.Equal(t, "chat", adminInfo["upstream_protocol"])
	assert.Equal(t, "anthropic_messages_to_openai_chat_completions", adminInfo["protocol_converter"])
	assert.Equal(t, "none", adminInfo["protocol_state_mode"])
