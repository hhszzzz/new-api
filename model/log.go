...
func formatUserLogs(logs []*Log, startIdx int, canViewModelRouting bool) {
	for i := range logs {
		...
		parseErr := common.DecodeJsonUseNumber(strings.NewReader(logs[i].Other), &otherMap)
...
func formatAdminLogs(logs []*Log) {
	for _, log := range logs {
		...
		err := common.DecodeJsonUseNumber(strings.NewReader(log.Other), &other)
		...
		log.Other = formatLogOtherJSON(common.MapToJsonStr(other), logOtherVisibilityAdmin)
	}
}
...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	otherMap := appendLogDiagnostics(c, channelId, isStream, other.Snapshot())
	otherStr := common.MapToJsonStr(withRequestedModelNameScope(otherMap, modelName))
	log := &Log{
		UserId:            userId,
		Username:          username,
		CreatedAt:         common.GetTimestamp(),
...
func RecordConsumeLog(c *gin.Context, userId int, params RecordConsumeLogParams) {
	...
	upstreamRequestId := c.GetString(common.UpstreamRequestIdKey)
	createdAt := common.GetTimestamp()
	otherMap := appendLogDiagnostics(c, params.ChannelId, params.IsStream, params.Other.Snapshot())
	otherStr := common.MapToJsonStr(withRequestedModelNameScope(otherMap, params.ModelName))
	log := &Log{
		UserId:            userId,
		Username:          username,
		CreatedAt:         createdAt,
...
func RecordTaskBillingLog(params RecordTaskBillingLogParams) {
	...
	log := &Log{
		...
		TokenId:   params.TokenId,
		Group:     params.Group,
		Other:     common.MapToJsonStr(withRequestedModelNameScope(params.Other.Snapshot(), params.ModelName)),
	}
	err := createLog(log)
	if err != nil {
