...
func TestModelLogWritersMarkRequestedModelNameProvenance(t *testing.T) {
	...
	consumeOther := NewLogOther()
	consumeOther.SetPublic("model_ratio", 1)
	errorOther := NewLogOther()
	errorOther.SetPublic("status_code", 500)
	taskOther := NewLogOther()
	taskOther.SetPublic("is_task", true)
	RecordConsumeLog(ctx, 41, RecordConsumeLogParams{
		ModelName: "requested-consume-model",
		Quota:     1,
		Other:     consumeOther,
	})
	RecordErrorLog(ctx, 41, 1, "requested-error-model", "", "diagnostic", 0, 0, false, "default", errorOther)
...
func TestLegacyLogOtherVisibilityIsRoleSeparated(t *testing.T) {
	...
	t.Run("user", func(t *testing.T) {
		...
		parsed, err := common.StrToMap(logs[0].Other)
		require.NoError(t, err)
		assert.NotContains(t, parsed, "request_path")
...
func TestLogFormattingPreservesLargeIntegerLexemes(t *testing.T) {
	...
	t.Run("unprivileged", func(t *testing.T) {
		...
		formatUserLogs(userLogs, 0, false)
		assert.JSONEq(t, unprivileged, userLogs[0].Other)
		assert.Contains(t, userLogs[0].Other, `"public_id":9007199254740993`)

		adminLogs := []*Log{{Other: unprivileged}}
		FormatAdminLogs(adminLogs)
		assert.JSONEq(t, unprivileged, adminLogs[0].Other)
		assert.Contains(t, adminLogs[0].Other, `"public_id":9007199254740993`)
	})
}
