...
func SearchModelsWithChannels(keyword, vendor, status, syncOfficial string, offset, limit int, sortOptions ...ModelSortOptions) ([]*Model, int64, error) {
	records, _, err := SearchModels(keyword, vendor, status, syncOfficial, 0, -1, sortOptions...)
	if err != nil {
		return nil, 0, err
	}
...
func (mi *Model) Update() error {
	return metadataTransaction(func(tx *gorm.DB) error { return mi.updateWithTx(tx) })
}
