...
func migrateDB() error {
	...
	if err := MigratePromptAuditDefaults(); err != nil {
		return err
	}
...
` + "`quota_5h_amount`" + ` bigint NOT NULL DEFAULT 0,
` + "`quota_weekly_amount`" + ` bigint NOT NULL DEFAULT 0,
` + "`created_at`" + ` bigint,
` + "`updated_at`" + ` bigint,
PRIMARY KEY (` + "`id`" + `)
)`
	...
	required := []sqliteColumnDef{
		...
		{Name: "quota_5h_amount", DDL: "`quota_5h_amount` bigint NOT NULL DEFAULT 0"},
		{Name: "quota_weekly_amount", DDL: "`quota_weekly_amount` bigint NOT NULL DEFAULT 0"},
