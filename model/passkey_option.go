...
func UpdatePasskeyDomainOptions(values map[string]string, preview bool, confirmation string) (*PasskeyDomainChange, error) {
	return updateOptionsBulk(values, preview, confirmation)
}

// preparePasskeyDomainOptions validates and normalizes the trust change inside
// the same transaction as every other option in the batch. The caller holds
// optionUpdateMu and passkeyOptionMutex and publishes only after commit.
func preparePasskeyDomainOptions(tx *gorm.DB, values map[string]string, preview bool, confirmation string) (*PasskeyDomainChange, error) {
	...
	for _, key := range []string{"ServerAddress", "passkey.legacy_rp_ids", "passkey.origins", "passkey.rp_id"} {
		if value, ok := values[key]; ok {
			...
			if key == "ServerAddress" {
				...
				handleConfigUpdate(key, value, false)
			}
		}
	}
}
