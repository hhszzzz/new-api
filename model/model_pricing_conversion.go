...
func legacyInputPricePerMillion(ratio float64) (decimal.Decimal, error) {
	if common.GetQuotaPerUnit() <= 0 || math.IsInf(common.GetQuotaPerUnit(), 0) || math.IsNaN(common.GetQuotaPerUnit()) {
	...
	return decimal.NewFromFloat(ratio).Mul(decimal.NewFromInt(1_000_000)).Div(decimal.NewFromFloat(common.GetQuotaPerUnit())), nil
}
