...
func TestResponseModelComparisonAndLog(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NotContains(t, other.Snapshot(), "response_model", "routing diagnostics must stay admin-only")
			if tc.returned == "" {
...
func TestResponseModelHandlersCaptureBeforeConversion(t *testing.T) {
	...
	chatStream := "data: " + `{"id":"chat_1","model":"returned","choices":[{"index":0,"delta":{"role":"assistant","content":"hello"},"finish_reason":"stop"}]}` + "\n\ndata: " + `{"id":"chat_1","choices":[],"usage":{"prompt_tokens":2,"completion_tokens":3,"total_tokens":5}}` + "\n\ndata: [DONE]\n\n"
...
func TestResponseModelDoesNotRecordSynthesizedModel(t *testing.T) {
	...
	_, apiErr := openai.OaiResponsesToChatBufferedStreamHandler(c, info, &http.Response{
		StatusCode: http.StatusOK, Header: make(http.Header), Body: io.NopCloser(strings.NewReader("data: {\"type\":\"response.completed\",\"response\":{\"status\":\"completed\",\"usage\":{\"input_tokens\":0,\"output_tokens\":0}}}\n\ndata: [DONE]\n\n")),
	})
	require.Nil(t, apiErr)
	assert.Nil(t, info.ResponseModel)
}
