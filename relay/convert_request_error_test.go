...
func TestImageRequestReservesFinalQuantityBeforeUpstream(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			common.SetContextKey(c, constant.ContextKeyChannelSetting, hostdto.ChannelSettings{PassThroughBodyEnabled: tc.passThrough})
			if tc.override != nil {
			...
			if tc.tiered {
				...
				info.TieredBillingSnapshot = &billingexpr.BillingSnapshot{BillingMode: "tiered_expr", ExprString: expr, ExprHash: billingexpr.ExprHashString(expr), GroupRatio: 1, QuotaPerUnit: common.GetQuotaPerUnit(), EstimatedImageCount: common.GetPointer(1)}
...
func TestImageHelperKeepsAdaptorClassifiedConvertError(t *testing.T) {
	...
	require.NotNil(t, apiErr)
	assert.Equal(t, http.StatusBadRequest, apiErr.StatusCode)
	assert.Equal(t, hosttypes.ErrorCodeInvalidRequest, apiErr.GetErrorCode())
	assert.True(t, hosttypes.IsSkipRetryError(apiErr), "other channels must not be asked to serve the same name")
