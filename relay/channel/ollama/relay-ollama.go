...
func ollamaEmbeddingHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *hosttypes.NewAPIError) {
	...
	if oResp.Error != "" {
		return nil, hosttypes.NewOpenAIError(fmt.Errorf("ollama error: %s", oResp.Error), hosttypes.ErrorCodeBadResponseBody, http.StatusInternalServerError)
	}
	...
	embResp := &dto.OpenAIEmbeddingResponse{Object: "list", Data: data, Model: info.PublicResponseModelName(), Usage: *usage}
