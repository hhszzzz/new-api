...
func TestCountBillableToolCallFunctionCallUsesFrozenPriceAndBillingModel(t *testing.T) {
	const priceKey = "snapshot_fn:private-model*"
	operation_setting.SetToolPriceForTest(priceKey, 5)
	frozenPrices := operation_setting.GetToolPriceSnapshot()
	operation_setting.SetToolPriceForTest(priceKey, 0)
	t.Cleanup(func() {
		operation_setting.DeleteToolPriceForTest(priceKey)
	})

	info := &RelayInfo{
		OriginModelName:   "public-model",
		BillingModelName:  "private-model-v1",
		ToolPriceSnapshot: frozenPrices,
	}
	info.CountBillableToolCall(dto.BuildInCallFunctionCall, "snapshot_fn")

	require.NotNil(t, info.ResponsesUsageInfo)
	require.Contains(t, info.ResponsesUsageInfo.BuiltInTools, "snapshot_fn")
	assert.Equal(t, 1, info.ResponsesUsageInfo.BuiltInTools["snapshot_fn"].CallCount)
}
