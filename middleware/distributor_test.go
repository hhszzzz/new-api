...
func TestChannelMatchesExpectedTaskPluginRejectsUnindexedLegacyChannel(t *testing.T) {
	...
	setupErr := SetupContextForSelectedChannel(c, &model.Channel{Type: constant.ChannelTypeJimeng}, "task-model", false)
...
func TestSharedEndpointRebindsToSelectedLegacyProvider(t *testing.T) {
	...
	require.Nil(t, SetupContextForSelectedChannel(c, vertexChannel, "task-model", false))
	...
	for _, locale := range []struct{ language, message string }{
		...
		for _, providerCount := range []int{1, 2} {
			t.Run(fmt.Sprintf("%s/%d_providers", locale.language, providerCount), func(t *testing.T) {
				...
				request := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"task-model","input":"hi"}`))
...
func TestSharedEndpointRebindsToSelectedType61Plugin(t *testing.T) {
	...
	channel.SetSetting(hostdto.ChannelSettings{TaskPluginKey: "unrelated"})
	assert.False(t, channelMatchesExpectedTaskPlugin(c, channel, "alpha"))
	channel.SetSetting(hostdto.ChannelSettings{TaskPluginKey: "beta"})
...
func TestChannelMatchesExpectedTaskPluginUsesNewAPIExtensions(t *testing.T) {
	...
	channel.SetSetting(hostdto.ChannelSettings{TaskPluginKey: "single", TaskExtendPluginKeys: []string{"ext-alpha", "ext-beta"}})
...
func TestSharedEndpointRebindsToBoundNewAPIExtension(t *testing.T) {
	...
	channel.SetSetting(hostdto.ChannelSettings{TaskExtendPluginKeys: []string{"unrelated"}})
	assert.False(t, channelMatchesExpectedTaskPlugin(pin("alpha"), channel, "alpha"))

	channel.SetSetting(hostdto.ChannelSettings{TaskExtendPluginKeys: []string{"beta"}})
	c := pin("alpha")
	require.Nil(t, SetupContextForSelectedChannel(c, channel, "task-model", false))
	...
	channel.SetSetting(hostdto.ChannelSettings{TaskExtendPluginKeys: []string{"alpha", "beta"}})
	c = pin("beta")
	require.Nil(t, SetupContextForSelectedChannel(c, channel, "task-model", false))
